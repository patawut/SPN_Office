<?php 
$return=array();
$query = $db->query("SELECT * FROM `product_type`  WHERE `status` <> '99'  ORDER BY `type_id` DESC ");
?>
    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">จัดการข้อมูลประเภทสินค้า</li>
            </ol>
        </nav>
    </div>
</div>
    ...
    <div class="card-body p-5">
        <div class="row no-gutters">
            <div class="col-md-8 order-md-1 mb-2 ">
                <h5 class="text-md-start text-center mb-0"><i class="fadeIn animated bx bx-cube"></i> จัดการข้อมูลประเภทสินค้า</h5>
            </div>
            ...
            <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-nowarp align-middle"><?=esc($row['type_name'])?></td>
                        <td class="text-nowarp align-middle"><?=esc($row['level'])?></td>
                        <td class="text-nowrap">
                            <select class="form-select <?=$row['status'] =="1"?"bg-success":"bg-danger"?> text-white" id="status" name="status" TypeID="<?=esc($row['type_id'])?>">
                        ...
                        <td class="text-center">
                            ...
                            <ul class="dropdown-menu">
                                <li> <a class="dropdown-item text-warning ControlEdit" tID="<?=esc($row['type_id'])?>"
                                        href="javascript:;"><i class="fadeIn animated bx bx-edit"></i>&nbsp;แก้ไขข้อมูล</a>
                                </li>
                                <li> <a class="dropdown-item text-danger ControlDelete" tID="<?=esc($row['type_id'])?>"
...
$(document).ready(function() {
    ...
    $('.ControlDelete').click(function(e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("./component/producttype/process", {
                        even: 'del',
                        id: id,
                        st: st
                    },
    ...
    $('.ControlEdit').click(function(e) {
        ...
        let id = $(this).attr('tID');
        var even = 'edit';
        $.post("./component/producttype/form", {
            even: even,
            id: id
        }, (data) => {
            $('#contentData').html(data);
